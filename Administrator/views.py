    ...
    if request.user.is_authenticated and request.user.is_admin:
        ...
        if request.method == "POST":
            ...
            if user and user.is_admin:
                ...
                return render(
                    request, "Administrator/administrator_login.html", context
                )
        else:
            msg = ""
...
def participant_manager(request):
    if request.user.is_authenticated and request.user.is_admin:
        participant_list = Participants.objects.values(
            "reg_no",
            "reg_no__fname",
            "reg_no__lname",
            "reg_no__email",
            "reg_no__contect_no",
            "reg_no__clg_id__clg_name",
            "reg_no__stream__stream_name",
            "remark",
            "total_payment",
            "remaining_payment",
            "paid_payment",
        ...
        for participant in participant_list:
            events[participant["reg_no"]] = Participation.objects.filter(
                reg_no=participant["reg_no"]
            ).values("event__event_name", "reg_status")
...
def participant_detail(request, reg_no):
    if request.user.is_authenticated and request.user.is_admin:
        participant = Participants.objects.filter(reg_no=reg_no).values(
            "reg_no",
            "reg_no__fname",
            "reg_no__lname",
            "reg_no__email",
        ...
        events = Participation.objects.filter(reg_no=reg_no).values(
            "event__event_name", "reg_status"
        )
...
def participant_edit(request, reg_no):
    if request.user.is_authenticated and request.user.is_admin:
        ...
        if request.method == "POST":
            participant_user_form = participant_user_model_form(
                data=request.POST, instance=participant_user
            )
            registers_form = registers_model_form(
                data=request.POST, instance=registration
            )
            if participant_user_form.is_valid() and registers_form.is_valid():
                ...
                context = {
                    "participant_user_form": participant_user_form,
                    "registers_form": registers_form,
                    "edit_form": True,
                    "participant": registration,
                    "reg_no": participant_user.reg_no,
                    "events": events,
                }
                return render(
                    request, "Administrator/participant_details.html", context
                )
        else:
            participant_user_form = participant_user_model_form(
                instance=participant_user
            )
    ...
    if request.user.is_authenticated and request.user.is_admin:
        if request.method == "POST":
            ...
            if addeventhead_form.is_valid():
                if not Event_Head.objects.filter(
                    reg_no=request.POST["reg_no"], event=request.POST["event"]
                ):
                    addeventhead_form.save()
                    committee_obj = Event_Committee.objects.get(
                ...
                eventhead_list = Event_Head.objects.all().values(
                    "reg_no",
                    "reg_no__committee_id",
                    "event__event_name",
                    "reg_no__reg_no__fname",
                    "reg_no__reg_no__lname",
                    "reg_no__reg_no__contect_no",
                    "reg_no__reg_no__email",
                    "isActive",
                    "event__event_id",
                )
                context = {
                    "eventhead_list": eventhead_list,
                    "addeventhead_form": addeventhead_form,
                }
    ...
    if request.user.is_authenticated and request.user.is_admin:
        if request.method == "POST":
            ...
            if addCoordinator_form.is_valid():
                ...
                coordinator_list = Coordinator.objects.all().values(
                    "reg_no",
                    "reg_no__committee_id",
                    "reg_no__reg_no__fname",
                    "reg_no__reg_no__lname",
                ...
                context = {
                    ...
                    "addCoordinator_form": addCoordinator_form,
                }
                return render(
                    request, "Administrator/coordinator_manager.html", context
                )
        else:
            coordinator_list = Coordinator.objects.all().values(
                "reg_no",
...
def event_committee_list(request):
    if request.user.is_authenticated and request.user.is_admin:
        committee = Event_Committee.objects.values(
            "reg_no",
            "committee_id",
            "reg_no__fname",
            "reg_no__lname",
...
def event_manager(request):
    if request.user.is_authenticated and request.user.is_admin:
        ...
        for event in eventlist:
            event_heads[event["event_id"]] = Event_Head.objects.filter(
                event=event["event_id"]
            ).values("reg_no__committee_id", "isActive")
...
def event_edit(request, event_id):
    if request.user.is_authenticated and request.user.is_admin:
        ...
        if request.method == "POST":
            ...
            editevent_form = event_model_form(
                request.POST, request.FILES, instance=event_obj
            )
            ...
            editevent_form = event_model_form(
                instance=event_obj, initial={"event_logo": event_obj.event_logo}
            )
            context = {"editevent_form": editevent_form, "editevent_id": event_id}
            return render(request, "Administrator/event_manager.html", context)
    else:
...
def event_delete(request, event_id):
    if request.user.is_authenticated and request.user.is_admin:
        if request.method == "POST":
            ...
            event_obj = Event.objects.get(event_id=event_id)
            context = {
                "deleteevent": event_obj.event_name,
                "deleteevent_id": event_id,
...
def event_participation_list(request, event_id):
    if request.user.is_authenticated and request.user.is_admin:
        participation = Participation.objects.filter(event=event_id).values(
            "reg_no", "reg_no__reg_no__fname", "reg_no__reg_no__lname", "reg_status"
        )
        event_name = Event.objects.filter(event_id=event_id).values("event_name")[0][
            "event_name"
        ]
...
def event_winners_list(request, event_id):
    if request.user.is_authenticated and request.user.is_admin:
        winners = Winner.objects.filter(event=event_id).values(
            "winner__reg_no",
            "winner__reg_no__reg_no__fname",
            "winner__reg_no__reg_no__lname",
            "position",
            "winning_certificate_issue",
            "event_head__reg_no__committee_id",
        )
        event_name = Event.objects.filter(event_id=event_id).values("event_name")[0][
            "event_name"
        ]
...
def payments_list(request):
    if request.user.is_authenticated and request.user.is_admin:
        payments = Payments.objects.values(
            "payment_id",
            "reg_no",
            "reg_no__reg_no__fname",
            "reg_no__reg_no__lname",
    ...
    if request.user.is_authenticated and request.user.is_admin:
        ...
        if request.method == "POST":
            ...
            if addcollage_form.is_valid():
                ...
                context = {
                    "collages": clg,
                    "addcollage_form": addcollage_form,
                    "clg_id": "None",
                }
    ...
    if request.user.is_authenticated and request.user.is_admin:
        ...
        if request.method == "POST":
            ...
            if editcollage_form.is_valid():
                ...
                context = {
                    "collages": clg,
                    "editcollage_form": editcollage_form,
                    "clg_id": clg_id,
                }
                return render(request, "Administrator/collage_manager.html", context)
        else:
    ...
    if request.user.is_authenticated and request.user.is_admin:
        ...
        if request.method == "POST":
            ...
            if editstream_form.is_valid():
                ...
                context = {
                    "streams": streamlist,
                    "editstream_form": editstream_form,
                    "stream_id": stream_id,
                }
                return render(request, "Administrator/stream_manager.html", context)
        else:
...
def profile_administrator(request):
    ...
    if request.user.is_authenticated and request.user.is_admin:
        userinfo = User.objects.filter(reg_no=reg_no).values(
            "reg_no",
            "fname",
            "lname",
            "email",
        ...
        committeeinfo = Event_Committee.objects.filter(reg_no=reg_no).values(
            "committee_id", "yearOfStudy"
        )
